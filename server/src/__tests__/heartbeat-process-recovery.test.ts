...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("queues exactly one retry when the recorded local pid is dead", async () => {
    ...
    // Terminal run cleanup releases the stale checkout lock; the queued retry may
    // already have started and adopted the checkout by the time the retry
    // executionRunId is visible.
